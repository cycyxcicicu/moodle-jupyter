...
until docker exec -i infra-postgres psql -U "$POSTGRES_ADMIN_USER" -d "$MOODLE_DB_NAME" -tAc "SELECT 1 FROM pg_tables WHERE tablename='mdl_config'" 2>/dev/null | grep -q 1; do
